      ...
      <el-form :model="formLabelAlign" inline>
        ...
        <el-form-item label="日期">
          <el-date-picker
            ...
            @visible-change="
              (visibility) =>
                handleVisibleChange(visibility, 'jingliu', 'month')
            "
            :disabled-date="(time) => disabledDate(time, 'jingliu', 'month')"
          />
          <el-date-picker
            ...
            @change="handleInputChange"
            @visible-change="
              (visibility) => handleVisibleChange(visibility, 'jingliu', 'year')
            "
            :disabled-date="(time) => disabledDate(time, 'jingliu', 'year')"
          />
...
function getAtmosphereData () {
  ...
  getTbFlowPageQuery({
    ...
    .then((res) => {
      ...
      if (res.response.value.code === 'SUCCESS') {
        tableData.value = res.response.value.body.records.map((item) => ({
          ...item,
...
function deleteSelectedRows () {
  ...
  ElMessageBox.confirm(
    ...
    .then(() => {
      ...
      deleteTbFlowById(requestObject)
        .then((res) => {
          if (res.response.value.code === 'SUCCESS') {
            getAtmosphereData()
