    ...
    <div class="form-container" v-if="current === 0">
      ...
      <div class="upload-container" v-if="submitType === 'file'">
        ...
        <div class="download-template">
          <a href="/templates/水位数据模板.xls" download>点击此处下载Excel模板</a>
        </div>
      </div>
      ...
      <el-form :model="form" label-width="120px" style="width: 40%; margin: 0 27%">
        <el-form v-if="submitType === 'form'" :model="form" label-width="120px" style="width: 100%; margin: 0">
          ...
          <el-form-item label="水位数据：" required>
            <el-input-number v-model="waterLevel" placeholder="请输入水位数据" min="0" controls-position="right"
...
import { getTimesByType, queryDeviceByMultiWord, waterLevelInsertByExcel, waterLevelInsertByForm } from '@/api/getData'
...
function handleConfirm() {
  if (submitType.value === 'file') {
    ...
    files.value.forEach((file) => {
      ...
      waterLevelInsertByExcel(formData)
        .then((res) => {
