    ...
    >
      ...
      <el-table-column label="操作" width="auto" fixed="right" align="center">
        <template #default="scope">
          <el-button @click="downloadWaterAreaImage(scope.row)"
            >水域面积识别</el-button
          >
          <el-image-viewer
            v-if="showPreview"
...
// 查看水域面积
async function downloadWaterAreaImage(row) {
 console.log(row);
  const loadingInstance = ElLoading.service(loadingoptions);
   try {
 // 调用API获取TIFF文件（确保 responseType: 'blob'）
   const response = await getWaterAreaImage({ fileUrl: row.originalFilepath });
    const blobData = new Blob([response.response.value], { type: 'image/tiff' });
 // 检查响应数据是否有效
  if (!blobData || blobData.size === 0) {
    throw new Error("服务器返回空数据");
  }
  console.log('response.data 类型：', typeof blobData);
  console.log('response.data instanceof Blob:', blobData instanceof Blob);
 ...
 // 创建下载链接
  ...
  link.href = downloadUrl;
    ...
    const fileName = `水域面积_${row.observationTime}_${row.deviceId || "未知位置"}.tif`;
    ...
    ElMessage.success("水域面积图片下载成功");
  } catch (error) {
    console.error("下载水域面积图片失败:", error);
    ElMessage.error("下载水域面积图片失败，请稍后重试");
  } finally {
    loadingInstance.close();
  }
}
