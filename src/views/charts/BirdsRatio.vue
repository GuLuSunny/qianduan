...
import { getBirdRate } from '@/api/getData'
...
function fetchBirdData () {
  ...
  getBirdRate()
    .then((response) => {
      ...
      if (res.code === 'SUCCESS') {
        chartTitle.value = res.body.month
        totalNum.value = res.body.sum
        birdData = res.body.data.map((item) => ({
          name: item.speciesId,
          value: item.num,
          percentage: item.percentage
        }))
